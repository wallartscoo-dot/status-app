...
// Sound attachment fields shared by both createStatusSchema (JSON) and
// uploadStatusSchema (multipart) — carries the "Use Sound" selection
// (spec section 9: attach audio + preserve trim range + preserve volumes).
const soundAttachmentFields = {
  soundId: z.string().uuid().optional(),
  soundStartSec: z.coerce.number().min(0).optional(),
  soundEndSec: z.coerce.number().min(0).optional(),
  soundVolume: z.coerce.number().min(0).max(1).optional(),
  originalVolume: z.coerce.number().min(0).max(1).optional(),
};
