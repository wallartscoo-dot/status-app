...
export interface UploadStatusInput {
  ...
  durationSec?: number;
  hashtags?: string[];
  // Sound Library attachment (spec section 9: "Use Sound" flow).
  soundId?: string;
...
export interface ApiSound {
  ...
  title: string;
  artist: string | null;
  artistRole: "ARTIST" | "RECITER" | null;
  artistAvatarUrl: string | null;
  category: SoundCategory;
  islamicSubcategory: IslamicSubcategory | null;
  genre: { key: string; label: string } | null;
  mood: { key: string; label: string; emoji: string | null } | null;
  surah: {
    number: number;
    nameArabic: string;
    nameTransliteration: string;
  ...
  usageCount: number;
  favoriteCount: number;
  source: "LICENSED" | "ROYALTY_FREE" | "PUBLIC_DOMAIN" | "USER_UPLOADED";
...
export interface ApiSoundMeta {
  genres: { key: string; label: string }[];
  moods: { key: string; label: string; emoji: string }[];
  islamicSubcategories: { key: IslamicSubcategory; label: string; emoji: string }[];
}

interface ListSoundsFilters {
  page?: number;
  limit?: number;
  category?: SoundCategory;
...
export const api = {
  ...
  statuses: {
    ...
    upload: async (input: UploadStatusInput): Promise<{ status: ApiStatus }> => {
      ...
      if (input.soundId) {
        form.append("soundId", input.soundId);
        if (input.soundStartSec !== undefined) form.append("soundStartSec", String(input.soundStartSec));
        if (input.soundEndSec !== undefined) form.append("soundEndSec", String(input.soundEndSec));
        if (input.soundVolume !== undefined) form.append("soundVolume", String(input.soundVolume));
        if (input.originalVolume !== undefined) form.append("originalVolume", String(input.originalVolume));
      }
  ...
  messages: {
    conversations: (page = 1) =>
      request<ApiPaginated<ApiConversation> & { totalUnreadCount: number }>(
        `/api/messages/conversations?page=${page}`,
        { auth: true }
      ),
    start: (username: string) =>
      request<{ conversation: ApiConversation }>(`/api/messages/start/${username}`, {
        method: "POST",
        auth: true,
      }),
    list: (conversationId: string, page = 1) =>
      request<ApiPaginated<ApiMessage>>(`/api/messages/conversations/${conversationId}/messages?page=${page}`, {
        auth: true,
      }),
    send: (conversationId: string, body: string) =>
      request<{ message: ApiMessage }>(`/api/messages/conversations/${conversationId}/messages`, {
        method: "POST",
        auth: true,
        body: JSON.stringify({ body }),
      }),
    markRead: (conversationId: string) =>
      request<void>(`/api/messages/conversations/${conversationId}/read`, { method: "PATCH", auth: true }),
  },
  sounds: {
    list: (filters: ListSoundsFilters = {}) =>
      request<ApiPaginated<ApiSound>>(`/api/sounds?${toQueryString(filters)}`, { auth: true }),
    trending: (filters: { page?: number; limit?: number; category?: SoundCategory } = {}) =>
      request<ApiPaginated<ApiSound>>(`/api/sounds/trending?${toQueryString(filters)}`, { auth: true }),
    search: (
      q: string,
      filters: { page?: number; limit?: number; category?: SoundCategory; islamicSubcategory?: IslamicSubcategory } = {}
    ) => request<ApiPaginated<ApiSound> & { query: string }>(`/api/sounds/search?${toQueryString({ q, ...filters })}`, { auth: true }),
    meta: () => request<ApiSoundMeta>("/api/sounds/meta", { auth: true }),
    surahs: () => request<{ surahs: ApiSurah[] }>("/api/sounds/quran/surahs", { auth: true }),
    byId: (id: string) => request<{ sound: ApiSound }>(`/api/sounds/${id}`, { auth: true }),
    videosForSound: (id: string, page = 1) =>
      request<ApiPaginated<ApiStatus>>(`/api/sounds/${id}/videos?page=${page}`, { auth: true }),
    favorite: (id: string) =>
      request<{ alreadyFavorited: boolean }>(`/api/sounds/${id}/favorite`, { method: "POST", auth: true }),
    unfavorite: (id: string) => request<void>(`/api/sounds/${id}/favorite`, { method: "DELETE", auth: true }),
    listFavorites: (page = 1) => request<ApiPaginated<ApiSound>>(`/api/sounds/favorites/mine?page=${page}`, { auth: true }),
  },
  downloads: {
