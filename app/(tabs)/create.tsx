...
export default function Create() {
  ...
  const { sound, trim, volumes, clear: clearSound } = useSoundSelection();
  const { isPlaying, toggle: togglePreview } = useAudioPreview(sound?.audioUrl);
    ...
    try {
      await api.statuses.upload({
        ...
        hashtags: hashtags
          ...
          .filter(Boolean),
        soundId: sound?.id,
        soundStartSec: sound && trim ? trim.startSec : undefined,
        soundEndSec: sound && trim ? trim.endSec : undefined,
        soundVolume: sound ? volumes.soundVolume : undefined,
        originalVolume: sound ? volumes.originalVolume : undefined,
      });
      track("status_published", { type: media.type, categoryKey, soundId: sound?.id });
    ...
    return (
      <Screen style={styles.center}>
        ...
        <AttachedSoundCard
          ...
          trim={trim}
          isPlaying={isPlaying}
          onTogglePreview={() => sound && togglePreview(trim?.startSec ?? 0, trim?.endSec ?? sound.durationSec)}
          onRemove={clearSound}
        />
    ...
    return (
      <Screen edges={["top"]}>
        <ScrollView showsVerticalScrollIndicator={false}>
          ...
          <Text style={[styles.fieldLabel, { color: theme.textSecondary }]}>Sound</Text>
          <AttachedSoundCard
            sound={sound}
            trim={trim}
            isPlaying={isPlaying}
  ...
  if (step === "preview" && media) {
    return (
      <Screen edges={["top"]}>
        ...
        {!!sound && (
          <Text style={[styles.previewMeta, { color: theme.accent }]}>
            🎵 {sound.title} — {sound.artist ?? "Unknown"}
            {trim ? ` (${Math.round(trim.endSec - trim.startSec)}s clip)` : ""}
          </Text>
        )}
...
function PickOption({
  ...
  return (
    <Pressable onPress={onPress}>
      <GlassSurface style={styles.pickOption} strength="subtle" elevated={false}>
...
/**
 * Shows the sound currently attached from the Sound Library flow (Open
 * Sound Library → Preview → Use Sound → Trim → Adjust Volume), or a CTA to
 * go add one. Rendered at every step of Create so the selection stays
 * visible right up to publish.
 */
function AttachedSoundCard({
  sound,
  trim,
  isPlaying,
  onTogglePreview,
  ...
  sound: import("@/services/api").ApiSound | null;
    ...
    return (
      <Pressable onPress={() => router.push("/sounds")}>
        <GlassSurface style={styles.pickOption} strength="subtle" elevated={false}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={[styles.pickLabel, { color: theme.textPrimary }]}>Add Sound</Text>
            <Text style={[styles.pickSublabel, { color: theme.textMuted }]}>Music, Trending, or Islamic Audio</Text>
          </View>
          <Ionicons name="chevron-forward" size={18} color={theme.textMuted} />
        </GlassSurface>
      </Pressable>
  ...
  const clipLen = trim ? Math.round(trim.endSec - trim.startSec) : sound.durationSec;

  return (
    <View style={[styles.soundAttachedRing, { borderColor: theme.accent }]}>
      <GlassSurface style={styles.soundAttached} strength="subtle" bordered={false} elevated={false}>
        <Pressable onPress={onTogglePreview} style={[styles.soundAttachedPlay, { backgroundColor: theme.accent }]}>
          <Ionicons name={isPlaying ? "pause" : "play"} size={16} color="#fff" />
        </Pressable>
        <View style={{ flex: 1, minWidth: 0 }}>
          <Text numberOfLines={1} style={[styles.pickLabel, { color: theme.textPrimary }]}>{sound.title}</Text>
          <Text numberOfLines={1} style={[styles.pickSublabel, { color: theme.textMuted }]}>
            {sound.artist ?? "Unknown"} · {clipLen}s clip{sound.category === "ISLAMIC" ? " · 🕌" : ""}
          </Text>
        </View>
        <Pressable onPress={() => router.push({ pathname: "/sounds/trim", params: { soundId: sound.id } })} hitSlop={8} style={{ marginRight: spacing.sm }}>
          <Text style={{ color: theme.accent, ...typography.tiny, fontWeight: "700" }}>Edit</Text>
        </Pressable>
...
const styles = StyleSheet.create({
  ...
  soundAttachedPlay: { width: 36, height: 36, borderRadius: 18, alignItems: "center", justifyContent: "center" },
